// Biddly Billing — e-mailverzending via Resend.
// De API-sleutel blijft server-side; de browser krijgt hem nooit te zien.
//
// Vereiste omgevingsvariabele in Netlify:  RESEND_API_KEY
// Optioneel:                               DEFAULT_FROM_EMAIL  (bv. no-reply@biddly.be)
...
exports.handler = async (event) => {
  ...
  if (!sender) {
    return json(400, { error: 'No sender address configured. Set it in Settings → Email Sending, or set DEFAULT_FROM_EMAIL.' });
  }
  ...
  try {
    ...
    let data = {};
    try { data = JSON.parse(raw); } catch (e) { /* Resend gaf geen JSON terug */ }

    if (!resp.ok) {
      // De foutmelding van Resend doorgeven: die zegt precies wat er mis is
      // (niet-geverifieerd domein, ongeldige sleutel, bijlage te groot, …).
